#!/usr/bin/env python3
"""
Comparison script for detections_ghassem.csv (Ghassem) vs Xmass_ghassem.fits (Alexis).
...
def compare_measurements(csv_data, fits_data, matched_indices):
    ...
    # 1. Redshift - EXCLUDED per user request
    ...
    if 'Flux_erg_cm2_s' in csv_matched.columns and 'Flux' in fits_matched.columns:
        results['flux'] = {
            ...
            'csv_err': csv_matched['Flux_Error'].values if 'Flux_Error' in csv_matched.columns else None,
            'fits_err': fits_matched['eFlux'].values if 'eFlux' in fits_matched.columns else None,
            'name': 'Flux',
            'unit': 'erg cm⁻² s⁻¹'
        }
    ...
    if 'Luminosity_erg_s' in csv_matched.columns and 'Lx' in fits_matched.columns:
        results['luminosity'] = {
            ...
            'csv_err': csv_matched['Luminosity_Error'].values if 'Luminosity_Error' in csv_matched.columns else None,
            'fits_err': fits_matched['eLx'].values if 'eLx' in fits_matched.columns else None,
            'name': 'Luminosity',
            'unit': 'erg s⁻¹'
        }
    ...
    if 'Temperature_keV' in csv_matched.columns and 'T' in fits_matched.columns:
        results['temperature'] = {
            ...
            'csv_err': csv_matched['Temperature_Error'].values if 'Temperature_Error' in csv_matched.columns else None,
            'fits_err': fits_matched['eT'].values if 'eT' in fits_matched.columns else None,
            'name': 'Temperature',
            'unit': 'keV'
        }
    ...
    if 'M200_Luminosity_Msun' in csv_matched.columns and 'M200' in fits_matched.columns:
        results['m200_lum'] = {
            ...
            'csv_err': csv_matched['M200_Luminosity_Error'].values if 'M200_Luminosity_Error' in csv_matched.columns else None,
            'fits_err': fits_matched['eM200'].values if 'eM200' in fits_matched.columns else None,
            'name': 'M200 (Luminosity-based)',
            'unit': 'M☉'
        }
    
    # 6. M200 (Temperature-based) - EXCLUDED per user request
    ...
    # 7. SNR (no errors typically available)
    ...
    if 'R200_Luminosity_kpc' in csv_matched.columns and 'R200_deg' in fits_matched.columns:
        ...
        cosmo = FlatLambdaCDM(H0=67.4, Om0=0.315)  # Planck18
        ...
        results['r200_lum'] = {
            ...
            'fits': r200_fits_kpc,
            'csv_err': None,  # R200 errors not directly available, would need to propagate from M200
            'fits_err': None,  # R200 errors not directly available
            'name': 'R200 (Luminosity-based)',
            'unit': 'kpc'
        }
    
    # 9. R200 (Temperature-based) - EXCLUDED per user request
...
def create_plots(results, output_dir):
    ...
    for idx, (key, data) in enumerate(results.items()):
        ...
        csv_err = None
        fits_err = None
        if 'csv_err' in data and data['csv_err'] is not None:
            csv_err_raw = data['csv_err'][valid]
            csv_err_valid = np.isfinite(csv_err_raw) & (csv_err_raw > 0)
            if np.any(csv_err_valid):
                csv_err = np.where(csv_err_valid, csv_err_raw, 0)
            else:
                csv_err = None
        if 'fits_err' in data and data['fits_err'] is not None:
            fits_err_raw = data['fits_err'][valid]
            fits_err_valid = np.isfinite(fits_err_raw) & (fits_err_raw > 0)
            if np.any(fits_err_valid):
                fits_err = np.where(fits_err_valid, fits_err_raw, 0)
            else:
                fits_err = None
        
        # Plot with error bars if available
        if csv_err is not None or fits_err is not None:
            # Use errorbar for both axes if errors available
            ax.errorbar(csv_valid, fits_valid, 
                       xerr=csv_err, yerr=fits_err,
                       fmt='o', alpha=0.6, markersize=4, 
                       capsize=2, capthick=1, elinewidth=0.5)
        else:
            # Fall back to scatter if no errors
            ax.scatter(csv_valid, fits_valid, alpha=0.6, s=30)
        
        # 1:1 line only (no fit line per user request)
        ...
        # Add parameter name to axes labels
        param_name = data['name']
        unit_str = f" ({data['unit']})" if data['unit'] else ""
        ax.set_xlabel(f"Ghassem {param_name}{unit_str}")
        ax.set_ylabel(f"Alexis {param_name}{unit_str}")
        ax.set_title(data['name'])
        ax.legend()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'ghassem_detections_fits_comparison_scatter_plots.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    for idx, (key, data) in enumerate(results.items()):
        ...
        # Get error bars if available (for x-axis only in residual plot)
        csv_err = None
        if 'csv_err' in data and data['csv_err'] is not None:
            csv_err_raw = data['csv_err'][valid]
            csv_err_valid = np.isfinite(csv_err_raw) & (csv_err_raw > 0)
        ...
        # Calculate residual errors (propagate errors)
        residual_err = None
        if csv_err is not None and 'fits_err' in data and data['fits_err'] is not None:
            fits_err = data['fits_err'][valid]
            fits_err_valid = np.isfinite(fits_err) & (fits_err > 0)
            csv_err_valid = np.isfinite(csv_err) & (csv_err > 0)
            both_valid = fits_err_valid & csv_err_valid
            
            if np.any(both_valid):
                # Error propagation: δ(residual) = sqrt((δfits/csv)^2 + (fits*δcsv/csv^2)^2) * 100
                residual_err = np.full_like(csv_valid, np.nan)
                residual_err[both_valid] = 100 * np.sqrt(
                    (fits_err[both_valid] / csv_valid[both_valid])**2 + 
                    (fits_valid[both_valid] * csv_err[both_valid] / csv_valid[both_valid]**2)**2
                )
                # Set invalid values to None for errorbar
                residual_err = np.where(np.isfinite(residual_err), residual_err, None)
        
        # Plot with error bars if available
        if csv_err is not None or residual_err is not None:
            ax.errorbar(csv_valid, residuals,
                       xerr=csv_err, yerr=residual_err,
                       fmt='o', alpha=0.6, markersize=4,
                       capsize=2, capthick=1, elinewidth=0.5)
        else:
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'ghassem_detections_fits_comparison_residual_plots.png', dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    output_dir = '/Users/gozalig1/Projects/compact-groups-xray-analysis/alexis_ghassem_measurments/ghassem_detections_fits_comparison_results'
    ...
    # Save statistics
    stats_df.to_csv(output_dir_path / 'ghassem_detections_fits_comparison_summary.csv', index=False)
    
    # Create text summary
    with open(output_dir_path / 'ghassem_detections_fits_comparison_summary.txt', 'w') as f:
        f.write("Detections_ghassem.csv vs FITS Comparison Summary\n")
        f.write("=" * 80 + "\n\n")
