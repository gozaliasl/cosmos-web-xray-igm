...
Groups are divided into four redshift bins: $0.05 \leq z < 0.85$
($N_{\rm nd} = 259$, $N_{\rm det} = 195$),
$0.85 \leq z < 1.30$ ($N_{\rm nd} = 261$, $N_{\rm det} = 93$),
...
optical position, rescaled to units of $r/r_{200}$, and averaged with equal
weighting after masking X-ray point sources and neighbours within
$2\,r_{200}$.
...
is derived from bootstrapping over the group sample.
All profiles are best described by a $\beta$-model with a fixed slope
$\beta = 0.5$ \citep[following][]{Popesso2025}, shown as the green solid
line in each panel.
...
within $0.2\,r_{200}$ in the redshift bin $0.85 \leq z < 1.30$, which
contains the largest number of groups with the highest signal-to-noise
stacked profiles.  The separation is also visible in the $0.05 \leq z < 0.85$
and $2.20 \leq z < 3.80$ bins at the $\sim$0.1--0.3~dex level, while the
$1.30 \leq z < 2.20$ bin shows large bootstrapping uncertainties due to
the lower individual group fluxes at these redshifts.
...
persists across all four redshift bins probed here, spanning
...
bin ($0.05 \leq z < 0.85$, $\langle z \rangle \approx 0.62$) are broadly
consistent with the \citet{Popesso2025} profiles at the lower end of their
mass range, with comparable surface brightness normalisation and $\beta$-model
slope once the different X-ray bands and cosmological dimming are accounted for.
...
\caption{Average X-ray surface brightness profiles of COSMOS-Web groups
  (CW-All) in four redshift bins.  In each panel, the blue shaded region
  shows the $1\sigma$ bootstrap band for X-ray detected groups; the red
  shaded region shows the corresponding band for non-detected groups.
  Solid lines indicate the smoothed stacked profile for each sub-population.
  The green dashed curve shows the best-fit $\beta$-model ($\beta = 0.5$)
  to the detected groups.  Vertical lines mark $R_{500}$ (orange dashed)
