...
\title{X-ray Properties of Galaxy Groups in COSMOS-Web to $z=3.7$: Characterizing the Hot Intragroup Medium with JWST and Chandra+XMM-Newton (Paper~I)}
...
\abstract{
Galaxy groups represent the most abundant environments for galaxies across cosmic time, yet their X-ray properties remain poorly characterized, especially at high redshifts ($z > 1.5$). We present a comprehensive X-ray analysis of 1,678 galaxy groups (the CW-All sample) identified in the COSMOS-Web field using the AMICO algorithm applied to JWST-selected galaxies from the COSMOS2025 catalog. Utilizing deep Chandra and XMM-Newton X-ray observations combined with multi-wavelength data from JWST NIRCam, we characterize the hot intragroup medium (IGM) properties including X-ray luminosity, temperature, and halo mass out to $z \sim 3.7$. Applying a uniform aperture-photometry pipeline with a detection threshold of SNR~$\geq 2.0$, we detect X-ray emission from 497 groups (29.6\%), with detected systems exhibiting median luminosities of $\log_{10}(L_{\rm X}/{\rm erg~s}^{-1}) \approx 42.9$ at a median redshift of $z \approx 1.1$. Stacking undetected groups across nine redshift bins yields high-significance composite detections out to $z \sim 3.7$, revealing steadily rising ensemble luminosities and halo masses consistent with $\log_{10}(M_{200}/M_{\odot}) \approx 12$--13. These results bridge X-ray and optical selection methods, establishing the COSMOS-Web group catalog as a unique laboratory for studying group physics and evolution across cosmic time. A companion paper (Paper~II; Gozaliasl et al., in preparation) presents the same analysis for the subset of 912 COSMOS-Web Hickson Compact Groups (CW-HCG; Hasinger \& Others, in preparation), enabling a controlled comparison between the general group population and compact group systems.
...
In this work (Paper~I of a two-paper series), we characterize the X-ray emission from the full COSMOS-Web galaxy group sample (CW-All; 1,678 systems from \citealt{Toni2025}) using a uniform analysis pipeline that applies identical photometric apertures, background subtraction, and calibration choices to every object. Our goals are to (i) quantify the detection rate of diffuse X-ray emission from JWST-identified groups and understand how it differs from X-ray-selected samples, (ii) measure fluxes, luminosities, and mass proxies for individual detections, and (iii) recover ensemble properties via stacking for groups below the individual detection threshold, following the methodology validated in recent stacking and scaling-relation studies of galaxy groups and centrals (e.g.\ \citealt{Anderson2015,PlanckCollaboration2013,Popesso2025,Robson2023,Marini2024}). By combining JWST's deep NIR imaging with the existing COSMOS X-ray data, we probe group X-ray properties across $0.1 \lesssim z \lesssim 3.7$, spanning the epoch when groups were actively assembling and their baryon content was being shaped by feedback processes. A companion paper (Paper~II; Gozaliasl et al., in preparation) applies the same pipeline to the subset of 912 COSMOS-Web Hickson Compact Groups (CW-HCG; Hasinger \& Others in preparation), providing a controlled comparison between the general group population and compact systems that may represent an earlier stage of group evolution or a distinct evolutionary track shaped by stronger tidal interactions.
...
\subsection{COSMOS-Web Hickson Compact Groups (Hasinger et al.\ in preparation)}

Within the COSMOS-Web footprint, 912 Hickson compact groups (hereafter CW-HCG) have been identified by Hasinger \& Others (in preparation) following the original selection criteria of \citet{Hickson1982}: at least four member galaxies within 3~mag of the brightest member, high angular surface density, and isolation from comparable-brightness neighbours. Compact groups are thought to represent dense, dynamically evolving systems, potentially at an earlier stage of group evolution or undergoing enhanced tidal interactions and galaxy merging \citep{MendesDeOliveira1994}.

The CW-HCG sample spans $0.12 \lesssim z \lesssim 3.83$ with a median redshift of $z \approx 0.97$. The same X-ray analysis pipeline described in Section~\ref{sec:analysis} is applied to CW-HCG in Paper~II (Gozaliasl et al., in preparation), which presents a full characterization of their X-ray properties, detection rates, stacked luminosities, and comparison with the CW-All population. In Paper~I we include the CW-HCG only as a brief comparison sample in the detection summary (Table~\ref{tab:detection}) and the redshift-distribution figure (Figure~\ref{fig:redshift_dist}).
...
Figure~\ref{fig:redshift_dist} presents the redshift distribution of the CW-All sample, which spans $0.08 \lesssim z \lesssim 3.70$ with a median redshift of $z \approx 1.1$. The CW-HCG comparison sample (N=912, $z \approx 0.97$ median) is overlaid for orientation. Both samples peak at $z \sim 1$--1.5, corresponding to the epoch of peak cosmic star formation and structure assembly.
...
The CW-All sample shows a detection rate that reflects the low surface brightness of intragroup gas at the survey depths. Table~\ref{tab:detection} summarises the key detection statistics for both CW-All and, for comparison, the CW-HCG sample (whose full X-ray analysis is presented in Paper~II).

We detect X-ray emission from 497 of 1,678 CW-All groups (29.6\%). Detected systems cluster around $z \sim 1.1$ and exhibit a median luminosity of $\log_{10}(L_{\rm X}/\mathrm{erg~s}^{-1}) \approx 42.97$ and median halo mass $M_{200} \approx 2.9 \times 10^{13}~M_{\odot}$, consistent with group-scale halos hosting established hot atmospheres. The detection fraction reflects both genuine low surface brightness at high redshift and the SNR~$\geq 2.0$ threshold adopted for this work (see \S\ref{sec:detection}).

The $L_{\rm X}$--redshift plane (Figure~\ref{fig:LX_z}) summarises individual detections and upper limits for the CW-All sample, with stacked median luminosities from Section~\ref{sec:stacking} over-plotted. Table~\ref{tab:detection} also lists CW-HCG statistics for reference; a detailed comparison of the two samples' detection properties is deferred to Paper~II.

\subsection{Stacking analysis}

Given the prevalence of non-detections, stacking is crucial for recovering ensemble X-ray properties of the CW-All population. The procedure described in Section~\ref{sec:stacking} delivers high-SNR composite detections in all nine redshift bins. Table~\ref{tab:stacking_all} lists the CW-All ensemble measurements (median redshift, number of sources, stacked SNR, flux, and $L_{\rm X}$ per bin). Stacked $M_{200}^{L_{\rm X}}$ values are included in the full electronic table.

Significant stacked detections are obtained in every bin, validating the presence of diffuse intragroup gas despite the low individual detection rate. Figure~\ref{fig:stacking} shows the stacked rest-frame luminosities, observed-frame fluxes, SNR, and $M_{200}^{L_{\rm X}}$ as a function of redshift, comparing stacks built from all groups in each bin (solid curve) to stacks restricted to individually detected systems (dashed). The close agreement between the two curves at $z \lesssim 2$ indicates that the stacked signals are representative of the full group population and not driven solely by bright outliers. Stacking results for the CW-HCG sample are presented in Paper~II.

\subsection{Summary of CW-All X-ray results}

Using a uniform X-ray analysis pipeline with a SNR~$\geq 2.0$ detection threshold, we detect X-ray emission from 497 of 1,678 CW-All groups (29.6\%). Detected systems span $0.1 \lesssim z \lesssim 3.7$ and exhibit a median luminosity of $\log_{10}(L_{\rm X}/\mathrm{erg~s}^{-1}) \approx 42.97$ and median temperature $T \sim 1.3$--1.4~keV, indicative of group-scale hot atmospheres. Halo masses derived from the weak-lensing calibrated $M$--$L_{\rm X}$ scaling relation peak around $M_{200} \sim 2.9 \times 10^{13}~M_{\odot}$ for individual detections. Stacking across nine redshift bins yields high-significance composite detections out to $z \sim 3.7$, with ensemble luminosities rising from $L_{\rm X} \sim 10^{42}$~erg~s$^{-1}$ at $z \approx 0.4$ to $\sim 10^{44}$~erg~s$^{-1}$ at $z \approx 3.2$, consistent with the expected growth in halo mass with redshift in our flux-limited stack. Stacked halo masses reach $\log_{10}(M_{200}/M_{\odot}) \approx 13$ in the highest-redshift bins. The full source-by-source photometry, detection flags, and physical properties are compiled in \texttt{xray\_catalog\_cwall.fits}. A parallel analysis of the 912 CW-HCG compact groups, including a direct comparison of their detection rates and stacked luminosities with the CW-All results, is presented in Paper~II (Gozaliasl et al., in preparation).
...
%-------------------------------------------------------------------
\section{Average intra-group X-ray surface brightness profiles}
\label{sec:avg_profiles}
%-------------------------------------------------------------------

We apply the stacking procedure described in Section~\ref{sec:radial_stack} to
the full CW-All sample to obtain average radial X-ray surface brightness
profiles for both X-ray detected and non-detected groups.
Following \citet{Popesso2025}, we stack all groups in each redshift bin without
distinguishing between detected and non-detected systems in the first instance,
then compare the two sub-populations explicitly to characterise the drivers of
scatter in the $L_{\rm X}$--$M_{200}$ relation.

\subsection{Redshift binning and profile construction}
\label{sec:profile_zbins}

Groups are divided into three redshift bins: $0.05 \leq z < 1.30$
($N_{\rm nd} = 520$, $N_{\rm det} = 288$),
$1.30 \leq z < 2.20$ ($N_{\rm nd} = 327$, $N_{\rm det} = 91$), and
$2.20 \leq z < 3.80$ ($N_{\rm nd} = 334$, $N_{\rm det} = 118$),
where $N_{\rm nd}$ and $N_{\rm det}$ are the numbers of non-detected
and detected groups per bin, respectively.
The lowest-redshift bin merges the two sub-intervals $0.05 \leq z < 0.85$
and $0.85 \leq z < 1.30$ to improve signal-to-noise while keeping the
high-redshift bins, which probe a qualitatively different regime of
group formation, at finer sampling.
Profiles are extracted in 2-arcsec annular bins out to $r_{200}$, the virial
radius of each group estimated from the stacking median $R_{500}$ and
$R_{500}/R_{200} \approx 0.65$.  The maps are re-centred on each group's
optical position, rescaled to units of $r/r_{200}$, and averaged with
inverse-variance weighting after masking X-ray point sources and
neighbours within $2\,r_{200}$.  The radial profile is then rebinned onto
12 logarithmically spaced bins between the inner resolution limit and
$r_{200}$ to reduce bin-to-bin noise; uncertainties are propagated via
inverse-variance weighting within each coarse bin.
The background is estimated from an outer annulus at $1.5$--$2.5\,R_{500}$
and subtracted from every stacked profile.
Radial profiles are shown in Figure~\ref{fig:profiles_comparison}, where
each panel corresponds to a redshift bin and the $1\sigma$ uncertainty band
is derived from the weighted standard error of the mean profile across all
groups in each bin.

\subsection{Detected versus non-detected groups}
\label{sec:profiles_det_vs_nondet}

Figure~\ref{fig:profiles_comparison} overlays the average profiles of
X-ray detected groups (blue shaded region) and non-detected groups (red
shaded region) in each redshift bin.  Consistent with the findings of
\citet{Popesso2024} and \citet{Popesso2025} in the eFEDS field at $z < 0.2$,
we find that detected groups in COSMOS-Web exhibit systematically brighter
and more centrally concentrated X-ray emission than their non-detected
counterparts at the same redshift.

The stacked surface brightness of detected groups exceeds that of
non-detected groups by $\Delta\log_{10}(\mathrm{SB}) \approx 0.4$~dex
within $0.2\,r_{200}$ in the lowest redshift bin ($0.05 \leq z < 1.30$,
$N = 808$ groups), which has the highest signal-to-noise owing to the
large number of stacked systems.  The separation remains visible in the
$1.30 \leq z < 2.20$ bin at the $\sim$0.3~dex level, while the
$2.20 \leq z < 3.80$ bin shows convergence between detected and
non-detected profiles within the measurement uncertainties, consistent
with the lower individual group fluxes at these redshifts.
This central brightness difference mirrors the result of
\citet{Popesso2025}, who find in the eFEDS field at $z < 0.2$ that
undetected groups have less than $33\pm15\%$ of the X-ray luminosity
concentrated within the inner $0.2\,r_{200}$, compared to $67\pm12\%$
for detected systems.  The profiles converge to similar levels near
$r_{200}$, indicating that the total gas content within the virial radius
differs less than its central concentration between the two populations.

The morphological difference between detected and non-detected groups
persists across all three redshift bins probed here, spanning
$0.05 \leq z \leq 3.80$.  This redshift leverage is substantially larger
than in previous studies based on low-redshift surveys
\citep[e.g.,][]{Popesso2024,Popesso2025}, and demonstrates that the
connection between central gas concentration and X-ray detectability
is already established at $z \sim 1$--3.

\subsection{Comparison with Popesso et al.\ (2025)}
\label{sec:profile_popesso_comparison}

\citet{Popesso2025} present average X-ray surface brightness profiles for
GAMA-selected galaxy groups in the eFEDS field in six halo mass bins spanning
$12.2 < \log(M_{200}/\mathrm{M_\odot}) < 14.0$ at $z < 0.2$.
Their profiles are expressed in units of $\log(L_{\rm X})\,[\mathrm{erg\,s^{-1}\,kpc^{-2}}]$
versus $\log(r/r_{200})$, and all are well described by $\beta$-models with
$\beta = 0.5$.

Our COSMOS-Web profiles probe a complementary regime: we reach
$z \sim 1$--3 and cover groups spanning a similar mass range but at
look-back times of 8--12~Gyr.  The profile shapes in our lowest-redshift
bin ($0.05 \leq z < 1.30$, $\langle z \rangle \approx 0.8$) are broadly
consistent with the \citet{Popesso2025} profiles at the lower end of their
mass range, with comparable surface brightness normalisation once
the different X-ray bands and cosmological dimming are accounted for.
At $z > 1$, both detected and non-detected profiles become progressively
shallower and more consistent with a flat background, reflecting both the
increased cosmological dimming of the hot gas emission and the lower overall
gas content expected in lower-mass, higher-redshift groups
\citep[see also][]{Sun2009,Lovisari2015}.

A key difference from \citet{Popesso2025} is that, while their analysis
brackets the profiles between detected and non-detected at fixed halo mass,
our comparison is performed at fixed redshift.  At the halo masses typical
of our sample ($M_{200} \sim 10^{13}\,\mathrm{M_\odot}$), AGN feedback is
expected to efficiently expel baryons beyond $r_{500}$, reducing the central
gas concentration and consequently suppressing X-ray detectability
\citep{Oppenheimer2021}.  The consistent detection of a concentration
difference between detected and non-detected groups across the full
redshift range $0.05$--$3.80$ suggests that this feedback-driven scatter
in the $L_{\rm X}$--$M_{200}$ relation is already in place at $z \sim 2$--3,
providing new observational constraints for hydrodynamical simulations.

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{figures/stacking_radial_profiles_comparison}
\caption{Average X-ray surface brightness profiles of COSMOS-Web groups
  (CW-All) in three redshift bins.  In each panel, the blue shaded region
  shows the $1\sigma$ uncertainty band for X-ray detected groups; the red
  shaded region shows the corresponding band for non-detected groups.
  Solid lines indicate the Gaussian-smoothed stacked profile for each sub-population.
  The band is the weighted standard error of the mean across all stacked groups,
  propagated through the logarithmic rebinning.
  Vertical lines mark $R_{500}$ (orange dashed)
  and $R_{200}$ (purple dotted).  At all redshifts, detected groups show
  brighter, more centrally concentrated profiles than non-detected groups,
  consistent with findings from the eFEDS field at $z < 0.2$
  \citep{Popesso2024,Popesso2025}.}
...
\bibitem[Popesso et al.(2025)]{Popesso2025} Popesso, P., Marini, I., Dolag, K., et al.\ 2025, A\&A, submitted [arXiv:2411.17128]

\bibitem[Oppenheimer et al.(2021)]{Oppenheimer2021} Oppenheimer, B.~D., Bogd\'an, \'A., Crain, R.~A., et al.\ 2021, ApJ, 912, L31
...
\caption{Redshift distribution of the COSMOS-Web group samples used in this work. Panel~(a): histogram of the CW-All sample (N=1678, blue), with the CW-HCG comparison sample (N=912, orange) overlaid; cumulative distributions are shown as dashed lines. Both samples peak at $z \sim 1$--1.5, with median redshifts of $z \approx 1.1$ (CW-All) and $z \approx 0.97$ (CW-HCG). Panel~(b): X-ray detection rate as a function of redshift for CW-All (blue); the CW-HCG detection rate is shown in Paper~II.}
...
\caption{Upper-limit diagnostics for the CW-All sample. Left panel: flux distribution histogram showing detected sources (blue), all upper limits (orange), and flagged upper limits (red) that fall below the first quartile of the detected flux distribution. Vertical dashed and dotted lines mark the Q1 and median of detected fluxes. Right panel: redshift distribution of flagged versus unflagged non-detections. Flagged sources are monitored for quality control; masking them changes the stacked fluxes by less than the statistical uncertainties. The equivalent diagnostic for CW-HCG is shown in Paper~II.}
\label{fig:upper_limits}
\end{figure*}

\begin{table}
\caption{Detection summary for COSMOS-Web group samples (SNR $\geq 2.0$). Median properties are computed for the detected subsample. The CW-All sample is the primary focus of this paper (Paper~I); a full analysis of CW-HCG is presented in Paper~II (Gozaliasl et al., in preparation). CW-HCG statistics here are included for reference only.}
\label{tab:detection}
\centering
...
CW-HCG$^{a}$ & 912  & 305 & 33.4 & $1.16 \times 10^{-15}$ & 42.88 & 0.97 & $2.82 \times 10^{13}$ \\
\hline
\end{tabular}
\tablefoot{$^{a}$ CW-HCG detection statistics are listed for comparison; the full CW-HCG X-ray analysis, stacking results, and comparison with CW-All are presented in Paper~II.}
\end{table}

\begin{figure*}
\centering
\includegraphics[width=0.8\textwidth]{figures/luminosity_redshift_cw_all}
\caption{Rest-frame 0.5--2.0~keV luminosity $L_{\rm X}$ as a function of redshift for the CW-All sample. Large open circles: individual detections (SNR~$\geq 2.0$). Grey points: non-detections with 95\% Bayesian upper limits; blue points: upper limits below the first quartile of the detected flux distribution (flagged). Orange squares: stacked median $L_{\rm X}$ per redshift bin (Table~\ref{tab:stacking_all}). This is the primary diagnostic for X-ray luminosity across the CW-All sample (see \S\ref{sec:LX_z}). The equivalent diagram for CW-HCG is shown in Paper~II.}
\label{fig:LX_z}
\end{figure*}
...
% Table of CW-HCG stacked measurements is presented in Paper II (Gozaliasl et al., in preparation).
...
\caption{Stacked X-ray properties of the CW-All sample as a function of redshift. Solid lines show stacks built from all groups in each bin; dashed lines show stacks restricted to individually detected systems. From left to right: rest-frame $L_{\rm X}$, observed-frame flux, stacked SNR, and halo mass $M_{200}^{L_{\rm X}}$. The close agreement between the two curves at $z \lesssim 2$ indicates that the stacked signals are representative of the full group population and not dominated by a small number of bright outliers. The equivalent stacking comparison for CW-HCG, including a direct CW-All vs.\ CW-HCG comparison panel, is presented in Paper~II.}
\label{fig:stacking}
\end{figure*}
...
% Fig. A.2 (CW-HCG radial profiles) is presented in Paper II (Gozaliasl et al., in preparation).
